...
Below each chord are some numbers. This is our own notation, and is not directly related to the standard notation for figured bass, which looks similar. Here the number on top is the scale degree of the chord, where 1 is the tonic, 5 is the chord of the fifth, etc. The number below that is the inversion of the chord. 0 for root position, 1 for first inversion, and 2 for second inversion.
...
So Harmony picks chords that it can "legally" make from that scale degree. It evaluates the chord in the context of the previous chord. The one that breaks the fewest rules for voice leading is then used. If no rules are broken, the some preferences can kick in.
