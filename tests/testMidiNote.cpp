...
//    assertEQ(ll, -2);
    int ll = mn.getLegerLine(ResolvedSharpsFlatsPref::Flats, false);
    assertEQ(ll, -2);
    ll = mn.getLegerLine(ResolvedSharpsFlatsPref::Sharps, false);
    assertEQ(ll, -2);
...
static void testMidiStaffCSharp() {
 ...
 //   int ll = mn.getLegerLine(false);
  //  assertEQ(ll, -2);
    ...
    ll = mn.getLegerLine(ResolvedSharpsFlatsPref::Sharps, false);
    assertEQ(ll, -2);
   // ll = mn.getLegerLine(ResolvedSharpsFlatsPref::DontCare, false);
   // assertEQ(ll, -2);
}
...
static void testMidiStaffC2() {
    ...
    const int ll = mn.getLegerLine(ResolvedSharpsFlatsPref::Sharps, false);
    assertEQ(ll, 5);
}
...
static void testMidiBassStaffA() {
    ...
    const int ll = mn.getLegerLine(ResolvedSharpsFlatsPref::Sharps, true);
    assertEQ(ll, 8);
}
...
static void testMidiStaffAll(bool bassStaff) {
    ...
    for (int i = 0; i < 128; ++i) {
        MidiNote mn(i);
        const int ll = mn.getLegerLine(ResolvedSharpsFlatsPref::Sharps, bassStaff);
        if (i != 0) {
...
static void testMidiStaffX() {
    ...
    const int ll2 = mn2.getLegerLine(ResolvedSharpsFlatsPref::Sharps, false);
    MidiNote mn3(MidiNote::MiddleC + 4);  // E
    const int ll3 = mn3.getLegerLine(ResolvedSharpsFlatsPref::Sharps, false);
}
...
void testMidiNote() {
  ...
  //  assert(false);
    ...
    testMidiIsBlackKey();
#if 0    // do these work? (no, that API was moved)
