...
public:
    ...
    float maxShift = -100;
    float minShift = 100;
    Outputs6 combine(const Outputs6 one) const {
        Outputs6 ret = one;
...
static float updateShift(const Inputs6& input, Outputs6& output, TestContext* context, float& accumulatedShift) {
    ...
    if (context) {
        ...
        if (ClockShifter6::llv) SQINFO("LFO = %f, ishift = %f", x, instantaneousShift);
    }
    output.lastShift = instantaneousShift;
    return instantaneousShift;
}

static Outputs6 runSub(const Inputs6& input, std::shared_ptr<ClockShifter6> shifter, TestContext* context) {
    if (ClockShifter6::llv) SQINFO("*** run Sub ");
    assert(input.isValid());

    Outputs6 output;

    // Originally all the offsets were integrated into this. Now the LFO is NOT integrated, so renamed this.
    ...
    while (samplesRemaining) {
        ...
        for (int i = 0; i < samplesThisTime; ++i) {
            ...
            if (ClockShifter6::llv) SQINFO("run about to clock it low samp %d", k + i);
...
static Outputs6 run(const Inputs6& _input, TestContext* context) {
    Outputs6 initOutput;
    // Step 1, setup.
    ...
    // Step 2, run forwards
    const Outputs6 outSub1 = runSub(adjustedInput, shifter, context);
    const Outputs6 out1 = initOutput.combine(outSub1);
    if (_input.afterwardsRunBackwards) {
        Inputs6 i = _input;  // will this cause an off by one?
        i.shiftPerSample *= -1;
        i.initialShift = out1.lastShift;
        // Step 3 (optional) run backwards.
        const Outputs6 out2 = runSub(i, shifter, context);
...
static void test0() {
    ...
    const Outputs6 o = run(in, nullptr);
}
...
static void testSlowDown(int cycles, int period, float shiftPerSample, int allowableJitter) {
    ...
    if (jitter > allowableJitter) {
        SQINFO("jitter bad, output=%s", output.toString().c_str());
...
static void testWithLFO(int cycles, int period, float lfoFreq, float lfoAmp, int allowedJitter) {
    SQINFO("----- testWithLFO %d, %d, %8f, %8f a=%d", cycles, period, lfoFreq, lfoAmp, allowedJitter);
    Inputs6 in;
    in.period = period;
    in.totalSamplesToTick = (cycles + in.initialShift) * in.period;
    ...
    const int jitter = std::abs(output.minSamplesBetweenClocks - output.maxSamplesBetweenClocks);
    SQINFO("jitter = %d allowable=%d", jitter, allowedJitter);
    if (jitter >= allowedJitter) {
...
static void testWithLFO() {
    ...
    cycles = 40;
    testWithLFO(cycles, period, lfoFreq, .4, 0);
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        testWithLFO(cycles, period, lfoFreq, .4, allowableJitter);
    }
...
void testFirst() {
   ...
   testSlowDown(5, 10, 0.0500000007, 2);
}
#endif